...
float circle_radius = 2.0f;               // radius of circle of flight [m]
float perp_speed_const = 1.3f;            // constant for perpendicular-to-body speed in circular motion
...
int32_t num_collision_detections = 0;         // number of detections on collision trajectory
int32_t num_lower_detections = 0;             // number of detections on lower trajectory
int32_t num_upper_detections = 0;             // number of detections on upper trajectory
...
static void obstacle_detection_cb(uint8_t __attribute__((unused)) sender_id, uint32_t __attribute__ ((unused)) stamp,
                                  int32_t flow_x, int32_t flow_y, int32_t flow_der_x,
                                  int32_t __attribute__((unused)) flow_der_y, float __attribute__((unused)) quality,
                                  float __attribute__((unused)) size_divergence)
{
  num_collision_detections = flow_x;
  num_lower_detections = flow_y;
  num_upper_detections = flow_der_x;
}
...
{ 
  ...
  AbiBindMsgOPTICAL_FLOW(OBSTACLE_DETECTION_ID, &obstacle_detection_ev, obstacle_detection_cb);
}
...
uint8_t flyBetterCircle(void)
{
  ...
  // Only consider the ABI message if we are on the right radius
  if (fabs(dist_to_center - circle_radius) < (0.05 * circle_radius)) {
    if (num_collision_detections > 7){
      if (num_upper_detections == 0){
        circle_radius = circle_radius + (0.3*circle_radius);
      }
      else if (num_lower_detections > num_upper_detections){
        circle_radius = circle_radius - (0.3*circle_radius);
      }
      else{
        circle_radius = circle_radius + (0.3*circle_radius);
      }
      oag_max_speed = (6.18 * circle_radius) / 9;
    }
  }
